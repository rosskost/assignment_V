...
get_venues_of_country_ticketmaster <- function (country_code = "DE", api_key) {
  ...
  number_pages <- as.data.frame(list.flatten(content_events_ger_II)) %>% 
      select(contains('totalPages')) %>% 
      slice(1)  # get number of pages to loop over
  
  df_venues_country = data.frame() # empty df we store data in
  for (i in 1:as.integer(number_pages)) {
    ...
    content_parsed <- fromJSON(content(response, as = "text"))
    
    df_content_i <- as.data.frame(list.flatten(content_parsed)) %>% 
      select(contains(
        c('venues.name','city.name','postalCode','address','venues.url','longitude','latitude')))
    
      df_venues_country = rbind(df_venues_country, df_content_i)
  }
...
```{r}
df_venues = get_venues_of_country_ticketmaster("DE", api_key)
